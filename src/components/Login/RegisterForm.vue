...
<v-card  >
    ...
    <v-card-text>
        <form @submit.prevent="register()">
            ...
            <v-text-field type="password" v-model="form.password_confirmation" label="ยืนยันรหัสผ่าน" required />

            <v-text-field v-model="form.firstname" label="ชื่อ" required />
            <v-text-field v-model="form.lastname" label="นามสกุล" required />
            
            <district-select v-if="form" @change="updateDistrictSelect" :valProvince="form.house_province" :valAmphur="form.house_amphur" :valDistrict="44"></district-select>

            <v-text-field label="รหัสไปรษณีย์" v-model="form.house_zipcode" hide-details /> <br>
