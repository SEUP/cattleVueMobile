...
<v-content class="font">
    ...
    <v-container class="bg-white" v-if="responseFrom">
        <div class="pd-4 box-green circle" v-if="success == true">
            <h4 class="wh"><span class="mdi mdi-checkbox-marked-circle"></span>&nbsp;การผสมพันธ์ุสำเร็จแล้ว</h4>
        </div>
        <br>
        ...
        <v-stepper v-model="form.status_id">
            ...
            <v-stepper-content step="1">
                <v-card class="elevation-0">
                    <introductionBreeding to="การเหนี่ยวนำการเป็นสัด" :remark="form.options.induction" v-model="form.induction" @change="form.options.induction = $event"></introductionBreeding>
                    <date-picker v-if="form.induction == '270100'" label="วัน/เดือน/ปี ที่เหนี่ยวนำ" :valDate='form.induction_date' @change="form.induction_date = $event" />
            ...
            <v-stepper-content step="2">
                <v-card class="elevation-0">
                    <date-picker label="วัน/เดือน/ปี ที่แสดงอาการเป็นสัด" :valDate="form.first_symptoms_date" @change="form.first_symptoms_date=$event" />
                </v-card>
            ...
            <v-stepper-content step="3">
                <v-card class="elevation-0">
                    <date-picker label="วัน/เดือน/ปี ที่ผสม" :valDate='form.breeding_date' @change="babyBirthday($event)" />
                    <!-----------------------วันที่ควรตรวจการกลับสัด-------------------------------->
                    <v-text-field label="วันที่ควรตรวจการกลับสัด" :value="dateback()" readonly hide-details class="pb-1 pr-2"></v-text-field>

                    <choice to="ประเภทการผสมพันธุ์" :remark="form.options.breed_type" v-model="form.breed_type" @change="form.options.breed_type = $event" />
                    <!--ผสมตามธรรมชาติ-->
                    <template v-if="form.breed_type == '090100'">
                        ...
                        <v-layout>
                            ...
                            <v-text-field label="ชื่อโคพ่อพันธุ์" hint="*กรณีโคพ่อพันธุ์ไม่อยู่ในระบบให้ใส่ชื่อแทน" persistent-hint v-model="form.male_name"></v-text-field>
                        </v-layout>
                        <choice to="พันธุ์โค" v-if="form.male_breed" :remark="form.options.male_breed" v-model="form.male_breed" @change="form.options.male_breed = $event" />
                    ...
                    <template v-if="form.breed_type == '090200'">
                        ...
                        <choice to="ผู้ให้บริการผสมเทียม" v-if="form.ivf_provider" :remark="form.options.ivf_provider" v-model="form.ivf_provider" @change="form.options.ivf_provider = $event" />
                    </template>
                </v-card>
            ...
            <v-stepper-content step="4">
                <v-card class="elevation-0">
                    ...
                    <!-----------------------วันที่ควรตรวจท้อง-------------------------------->
                    <v-text-field v-if="form.reversal == '280100'" label="วันที่ควรตรวจท้อง" :value="checkBellyDay()" readonly hide-details class="pb-1 pr-2"></v-text-field>
            ...
            <v-stepper-content step="5">
                <v-card class="elevation-0">
                    <date-picker label="วัน/เดือน/ปี ที่ตรวจท้อง" :valDate="form.check_date" v-model="form.check_date" @change="form.check_date = $event" />
            ...
            <v-stepper-content step="6">
                <v-card class="elevation-0">
                    ...
                    <div class="pt-3">
                        ...
                        <date-picker label="วัน/เดือน/ปี ที่คลอดจริง" :valDate="form.real_birth_date" v-model="form.real_birth_date" @change="form.real_birth_date = $event" />
...
export default {
    ...
    methods: {
        ...
        async getCatteleById(cattle) {
            console.log('Choose cattle', this.forms);
            let data = await this.$store.dispatch("manageDef/getDataById", {
                cattle_id: this.forms.cattle_id,
                id: this.forms.id
            });
            delete data['edit_mode'];
            if (data.status_id == 1) {
                let check = confirm('การผสมพันธุ์ครั้งนี้ได้สิ้นสุดกระบวนการแล้ว คุณแน่ใจใช่ไหมที่จะแก้ไขข้อมูล');
                if (!check) {
